...
abstract class core {
  ...
  static getProds() async => convertData(await db.getProds(), (j) => Prod.fromJSON(j)).cast<Prod>();
  static getProdUnits(int prodID) async => convertData(await db.getProdUnits(prodID), (j) => Unit.fromJSON(j)).cast<Unit>();

  static addProd(Prod prod) async => (await api.addProd(prod.toJSON()))['status'] == 200 && await sync();
  static updProd(Prod prod) async => (await api.updProd(prod.id!, prod.toJSON()))['status'] == 200  && await sync();
  static delProd(Prod prod) async => (await api.delProd(prod.id!))['status'] == 200  && await sync();


  // --- UNITS --- //

  static getUnits() async => convertData(await db.getUnits(), (j) => Unit.fromJSON(j)).cast<Unit>();
  ...
  static getProds4Order() async => convertData(await db.getProds4Order(), (j) => Prod.fromJSON(j)).cast<Prod>();

  // --- AGENTS --- //

  static getAgents() async => convertData(await db.getAgents(), (j) => Agent.fromJSON(j)).cast<Agent>();

  static addAgent(Agent agent) async => (await api.addAgents(agent.toJSON()))['status'] == 200 && await sync(); // db.insert(agent) > 0;
  static updAgent(Agent agent) async => (await api.updAgents(agent.id!, agent.toJSON()))['status'] == 200 && await sync(); // db.update(agent) > 0;
  static delAgent(Agent agent) async => (await api.delAgents(agent.id!))['status'] == 200 && await sync(); // db.delete(agent) > 0;
