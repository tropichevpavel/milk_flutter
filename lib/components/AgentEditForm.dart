...
class AgentEditForm extends StatefulWidget {
	...
	AgentEditForm(Agent? agent, this.ui, {Key? key}) :
		...
		child: Container(
			...
			child: Column(
				...
				TextFormField(
					...
					onChanged: (text) =>
						setState(() => widget.agent.name = text),
				...
				TextFormField(
					initialValue:
						widget.agent.id == null ? '' : '${widget.agent.phone}',
					onChanged: (text) =>
						setState(() => widget.agent.phone = int.parse(text)),
					textInputAction: TextInputAction.next,
					decoration: const InputDecoration(
				...
				TextFormField(
					...
					onChanged: (text) =>
						setState(() => widget.agent.adress = text),
				...
				Container(
					...
					child: Row(
						...
						children: [
						  ...
						  LoadButton(
							  onTap: widget.agent.id == null
								  ? () => core.addAgent(widget.agent)
								  : () => core.updAgent(widget.agent),
							  data: widget.ui,
							  ui: () => Navigator.pop(context),
							  disabled: widget.agent.name.isEmpty ||
								  widget.agent.phone == 0 ||
								  widget.agent.adress.isEmpty)
						]))
			  ])));
}
