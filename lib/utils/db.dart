...
abstract class db {
  ...
  static getProdUnits(int prodID) => get('SELECT unitID AS id, unitName AS name, puID, puPrice AS price FROM (SELECT puID, unitIDfk FROM prodUnits WHERE prodIDfk = $prodID AND puStatus = 1) AS pu LEFT JOIN units ON unitID = unitIDfk LEFT JOIN (SELECT prodUnitPrices.puIDfk, prodUnitPrices.puPrice FROM prodUnitPrices, (SELECT puIDfk, MAX(pupDate) AS pupDate FROM prodUnitPrices GROUP BY puIDfk) AS maxDate WHERE prodUnitPrices.puIDfk = maxDate.puIDfk AND prodUnitPrices.pupDate = maxDate.pupDate) AS pup ON puID = puIDfk;');
